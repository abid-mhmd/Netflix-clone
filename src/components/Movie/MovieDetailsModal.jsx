...
import { getMovieTrailer, getRelatedMovies } from "../../services/movieService";
...
function MovieDetailsModal({ movie, onClose }) {
  ...
  const [showUnavailable, setUnavailable] = useState(false);
  const { watchlist, addToWatchlist, removeFromWatchlist } = useWatchlist();

  const isInWatchlist = watchlist.some((item) => item.id === activeMovie.id);

  async function handlePlay() {
    const trailer = await getMovieTrailer(activeMovie.id);
    if (!trailer) {
      setUnavailable(true);
      return;
    }
  ...
  return (
    <>
          ...
          <div className="relative">
            ...
            <div className="absolute !bottom-8 !left-8 !right-8">
              ...
              <div className="flex !gap-5 items-center">
                <button
                  onClick={handlePlay}
                  className="flex items-center !gap-2 bg-white text-black !px-6 !py-2 rounded font-bold hover:bg-white/80 transition active:scale-95"
                >
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-full border flex items-center justify-center transition active:scale-95 border-zinc-400 bg-zinc-900/60 text-white hover:border-white hover:bg-zinc-800/80"
                >
                  {isInWatchlist ? (
      ...
      {showUnavailable && (
        <TrailerUnavailableModal onClose={() => setUnavailable(false)} />
      )}
    </>
  );
}
